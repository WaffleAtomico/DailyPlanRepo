...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 30-07-2024 a las 21:40:05
...
CREATE DEFINER=`root`@`localhost` EVENT `invitationsCancel` ON SCHEDULE EVERY 1 MINUTE STARTS '2024-07-30 13:32:54' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
    DECLARE rem_id INT;
    DECLARE done INT DEFAULT 0;
    ...
    reminder_loop: LOOP
        FETCH reminder_cursor INTO rem_id;
        IF done THEN
            LEAVE reminder_loop;
        END IF;

        -- Delete invitations associated with the identified reminder
        DELETE FROM invitations WHERE reminder_id = rem_id;
