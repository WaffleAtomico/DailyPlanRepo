...
import { formatDate, timeFormatHHMMSS, timeFormatHHMMSSString } from '../../../utils/timeFormat';
...
import { calculateTimeDifference } from '../../../utils/timeFormat';
import { calculatePunctuality, getPuntualityByUserId, updatePuntuality } from '../../../utils/validations/puntuality';
import { getCountReminders } from '../../../utils/validations/reminders';
import { deactivateReminder } from '../../../utils/validations/reminders';
...
*/
    ...
    useEffect(() => {
        if (blocks && blocks.length > 0) {
            // Find the first block with unconfirmed objectives
    ...
    useEffect(() => {
        ...
        if (unconfirmed.length === 0 && totalTimeLeft > 0) {
            handleNextBlock();
        }
    }, [objectiveList]);
...
// Define the completeBlock function
const completeBlock = (updatedBlocks) => {

    console.log("se completo el bloque", updatedBlocks);
    ...
    const diferenceTime = calculateTimeDifference(blocks);
    const result = calculatePunctuality(diferenceTime);
    getCountReminders(formatDate(new Date()), id_user).then(response => {
        console.log("Reminders count response:", response);
        const reminder_count = response.reminder_count;
    
        getPuntualityByUserId(id_user).then(response => {
            console.log("Punctuality by user response:", response);
            response.punt_num_rem = reminder_count;
    
            if (response.punt_percent_rem === 0) {
                response.punt_num_rem = result;
                console.log("Punt percent was zero, new punt_num_rem:", response.punt_num_rem);
            } else {
                response.punt_percent_rem = (result + response.punt_percent_rem) / 2;
                console.log("Updated punt_percent_rem:", response.punt_percent_rem);
            }
    
            updatePuntuality(response).then(response => {
                console.log("Punctuality update response:", response);
                deactivateReminder(blocks[0].reminder_id).then(response => {
                    console.log("Deactivate reminder response:", response);
                    console.log("Puntuality updated successfully");
                }).catch(error => {
                    console.log("Could not deactivate reminder", error);
                });
            }).catch(error => {
                console.log("Could not update punctuality", error);
            });
        }).catch(error => {
            console.log("Could not get today's punctuality:", error);
        });
    }).catch(error => {
        console.log("Could not get reminders count:", error);
    });
