...
const createUser = (req, res) => {
    const query = {
        ...
        values: [[req.body.user_mail, req.body.user_name, req.body.user_password, req.body.user_number, req.body.user_status]],
    };
    db.query(query.sql, query.values, (err, data) => {
        if (err) {
...
const updateUser = (req, res) => {
    const query = {
        ...
        values: [req.body.user_mail, req.body.user_name, req.body.user_password, req.body.user_number, req.body.user_id, req.body.user_status],
    };
...
const userExistsByNumber = (req, res) => {
    ...
    db.query(query.sql, query.values, (err, data) => {
        if (err) {
            return res.json({ message: 'Error verifying user by number', error: err });
        }
        if (data.length > 0) {
            return res.json({ exists: true });
        } else {
