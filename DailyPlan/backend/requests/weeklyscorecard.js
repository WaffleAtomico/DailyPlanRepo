...
const getWeeklyScorecardForUser = (req, res) => {
  const query = {
  ...
  db.query(query.sql, query.values, (err, data) => {
      if (err) {
          return res.json({ message: "Error getting weeklyscorecard for user", error: err });
      }
      return res.json(data);
  });
};

const isUserWeeklyScorecard = (req, res) => {
  const query = {
  ...
  db.query(query.sql, query.values, (err, data) => {
      if (err) {
          return res.json({ message: "Error checking weeklyscorecard limit", error: err });
      }
